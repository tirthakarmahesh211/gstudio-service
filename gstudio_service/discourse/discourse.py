...
GROUP_DELETE =        (requests.delete, "/admin/groups/{id}", None)
...
PAGE_GET =(requests.get, "/t/{id}.json", None)
...
class Group(object):
	...
	def get_group_params(self):
		# print(self.default_group_params)
		self.default_group_params.update(self.params)
		return self.default_group_params
...
class Discourse(object):
	...
	def request(self, function, url, member, params=None, type=None, throwOnFail=True):
		if not params:
			params = {}
		if self.apiName:
		...
		if type == "group":
			if function == requests.delete or function == requests.put:
				url=str(self.main_url+GROUP_GET[1]).format(name=params["group[name]"])
				response = requests.get(url,params=params)
				json_data = response.json()
				if json_data.get("group"):
					ID = json_data["group"]["id"]
					url = str(URL).format(id=ID)
					response = function(url,params=params)
					return response
				request_delete = True
			if URL and not request_delete:
				if params.get("group[name]"):
					url = str(URL).format(name=params["group[name]"])
				elif params.get("group[id]"):
					url = str(URL).format(id=params["group[id]"])
				else:
					url= URL
			if requests.put == function and "group[name]" in params:
			...
			if function == requests.put:
				...
				response = function(url,params=params)
				return response
			elif function == requests.delete:
				url=str(self.main_url+PAGE_DELETE[1]).format(id=params["id"])
				response = function(url,params=params)
				return response
			elif function == requests.get:
				url=str(self.main_url+PAGE_GET[1]).format(id=params["id"])
				response = function(url,params=params)				
			else:
				url = URL
				print(url)
			...
			# print(params)
			print
			if requests.post == function:
				response = function(url,files=params["files[]"],params=params)
			# print(response)
			return response
	...
	def groups(self,type):
		return self.request(*GROUPS_GET,type=type)
	
	def group(self, name, type):
		params = {"group[name]":name,"type":type}
		return self.request(*GROUP_GET,params=params,type=type)

	def add_group(self,params,type):
		params = Group(params).get_group_params()
		return self.request(*GROUP_ADD,params=params,type=type)

	def update_group(self,params,type):
		return self.request(*GROUP_PUT,params=params,type=type)

	def delete_group(self,params,type):
		return self.request(*GROUP_DELETE,params=params,type=type)

	def add_page(self,params,type):
		return self.request(*PAGE_ADD,params=params,type=type)

	def update_page(self,params,type):
		return self.request(*PAGE_UPDATE,params=params,type=type)

	def delete_page(self,params,type):
		return self.request(*PAGE_DELETE,params=params,type=type)
